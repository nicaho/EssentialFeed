        ...
        sut.retrieve { firstResult in
            sut.retrieve { secondResult in
                ...
                case (.empty, .empty):
                    break
                default:
                    XCTFail("Expected retrieving twice from empty cache to deliver same emptye result, got \(firstResult) and \(secondResult) instead")
                }
    ...
    private func expect(_ sut: CodableFeedStore, toRetirve expectedResult: RetrieveCacheFeedResult, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.retrieve { retrieveResult in
            ...
            case let (.found(expectedFeed, expectedTimestamp), .found(retrieveFeed, retrieveTimestamp)):
                XCTAssertEqual(expectedFeed, retrieveFeed)
                XCTAssertEqual(expectedTimestamp, retrieveTimestamp)
            default:
