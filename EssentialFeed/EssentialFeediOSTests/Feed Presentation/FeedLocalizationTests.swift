...
//
//  Created by nicaho on 2024/10/14.
//
...
final class FeedLocalizationTests: XCTestCase {
    func test_localizedStrings_haveKeysAndValuesForAllSupportedLocalizations() {
        ...
        localizationBundles.forEach { (bundle: Bundle, localization: String) in
            localizedStringKeys.forEach { key in
                ...
                if localizedString == key {
                    ...
                    XCTFail("Missing \(language) (\(localization) localized string for keys: '\(key)' in table: '\(table)'")
                }
            }
        }
    }
